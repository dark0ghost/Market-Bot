...
Документация по внутреннему API AI Trade Bot.
...
Все API методы возвращают `Result<T, E>` где `E` это `anyhow::Error`.
...
pub async fn some_operation(&self) -> Result<()> {
    ...
    Err(anyhow::anyhow!("Ошибка операции"))
}
```

## Логирование

Используется crate `log`:
