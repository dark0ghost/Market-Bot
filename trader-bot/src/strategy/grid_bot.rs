...
use crate::strategy::{GridStrategy, GridExecutor};
...
impl GridBot {
    ...
    pub async fn run(&mut self) -> Result<()> {
        info!("Запуск Grid бота для {} ({})", self.config.ticker, self.config.figi);
        ...
        let current_price = match self.market_data_service.get_last_price(&self.config.figi).await {
            Ok(p) => p,
            Err(e) => {
        ...
        match executor.initialize_grid(current_price).await {
            Ok(results) => {
                info!("Grid сетка инициализирована, размещено ордеров: {}", results.len());
            }
            Err(e) => {
        ...
        loop {
            ...
            if let Some(ref mut executor) = self.executor {
                ...
                let new_price = match self.market_data_service.get_last_price(&self.config.figi).await {
                    Ok(p) => p,
                    Err(e) => {
